  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          const isCompany = req.url.includes('/company');
          const { default: handler } = await import(
            isCompany ? './api/enrichment/company.js' : './api/enrichment/signals.js'
          );
...
function satellitesApiPlugin(): Plugin {
  return {
    name: 'satellites-api',
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        if (!req.url?.startsWith('/api/satellites')) return next();

        try {
          const { default: handler } = await import('./api/satellites.js');
          const port = server.config.server.port || 5173;
          const url = new URL(req.url, `http://localhost:${port}`);
          const headers: Record<string, string> = {};
          for (const [key, value] of Object.entries(req.headers)) {
...
function reverseGeocodePlugin(): Plugin {
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        if (!req.url?.startsWith('/api/reverse-geocode')) return next();
...
function routeApiPlugin(): Plugin {
  return {
    name: 'route-api',
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        if (!req.url?.startsWith('/api/route/')) return next();

        try {
          const { default: handler } = await import('./api/route/alternate.js');
          const port = server.config.server.port || 5173;
          const url = new URL(req.url, `http://localhost:${port}`);
...
function osintApiPlugin(): Plugin {
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        if (!req.url?.startsWith('/api/osint/')) return next();

        try {
          const { default: handler } = await import('./api/osint/dork-cxo.js');
          const port = server.config.server.port || 5173;
          const url = new URL(req.url, `http://localhost:${port}`);
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          const { default: handler } = await import('./api/sentiment/analyze.js');
          const port = server.config.server.port || 5173;
          const url = new URL(req.url, `http://localhost:${port}`);
