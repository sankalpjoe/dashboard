...
export async function fetchUnhcrPopulation(): Promise<UnhcrFetchResult> {
  const data = await breaker.execute(async () => {
    ...
    // Inject India-specific stats (2024/2025 data: 47k UNHCR registered, ~240k total refugees, 5.4M disaster IDPs)
    const india = summary.countries.find(c => c.code === 'IND' || c.name.includes('India'));
    if (india) {
      india.refugees = 47183;        // UNHCR Registered (Feb 2025)
      india.asylumSeekers = 10000;   // Estimated
      india.idps = 5400000;         // IDMC 2024 Disaster Displacement
      india.hostTotal = 240000;      // Total refugees + asylum seekers (comprehensive)
      india.totalDisplaced = india.refugees + india.asylumSeekers + india.idps;
    } else {
