...
export async function fetchCachedTheaterPosture(signal?: AbortSignal): Promise<CachedTheaterPosture | null> {
  ...
  fetchPromise = (async () => {
    try {
      ...
      console.warn('[CachedTheaterPosture] Backend fetch failed, trying client-side fallback:', error);

      try {
        // Fallback to client-side aggregation
        const fallbackData = await calculateTheaterPostureClientSide();
        if (fallbackData && fallbackData.postures.length > 0) {
          cachedPosture = fallbackData;
          lastFetchTime = Date.now();
          // Don't save fallback to persistent storage to avoid poisoning cache with partial client data
          return cachedPosture;
        }
      } catch (fallbackError) {
        console.error('[CachedTheaterPosture] Client-side fallback also failed:', fallbackError);
      }

      return cachedPosture; // Return stale cache on total error
    } finally {
      fetchPromise = null;
    }
  })();
 ...
 * Aggregates live data from military flight and vessel services.
 */
async function calculateTheaterPostureClientSide(): Promise<CachedTheaterPosture | null> {
  try {
    const { fetchMilitaryFlights, fetchMilitaryVessels, getTheaterPostureSummaries, recalcPostureWithVessels } = await import('./index');

    console.log('[CachedTheaterPosture] Calculating posture client-side...');
    ...
    // 2. Aggregate aircraft into theaters
    const postures = getTheaterPostureSummaries(flights.flights);

    // 3. Merge vessels and calculate levels
    for (const p of postures) {
      if (!p.bounds) continue;
      const theaterVessels = vessels.filter(v =>
        v.lat >= p.bounds!.south && v.lat <= p.bounds!.north &&
        v.lon >= p.bounds!.west && v.lon <= p.bounds!.east
      );

      p.destroyers = theaterVessels.filter(v => v.vesselType === 'destroyer').length;
      p.frigates = theaterVessels.filter(v => v.vesselType === 'frigate').length;
      p.carriers = theaterVessels.filter(v => v.vesselType === 'carrier').length;
      p.submarines = theaterVessels.filter(v => v.vesselType === 'submarine').length;
      p.patrol = theaterVessels.filter(v => v.vesselType === 'patrol').length;
      p.auxiliaryVessels = theaterVessels.filter(v =>
        ['auxiliary', 'special', 'amphibious', 'icebreaker', 'research'].includes(v.vesselType)
      ).length;
      p.totalVessels = theaterVessels.length;
    }

    recalcPostureWithVessels(postures);

    const data: CachedTheaterPosture = {
      postures,
      totalFlights: flights.flights.length,
      timestamp: new Date().toISOString(),
      cached: false,
      stale: false
    };

    console.log(`[CachedTheaterPosture] Client-side calculation complete: ${postures.length} theaters tracked`);
    return data;
  } catch (err) {
    console.error('[CachedTheaterPosture] Client-side calculation failed:', err);
    return null;
  }
}
