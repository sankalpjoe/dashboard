...
export async function fetchAllFires(_days = 7): Promise<FetchResult> {
  ...
  let detections = response.fireDetections;

  // Fallback to NASA EONET if FIRMS is unavailable or returns no data
  if (detections.length === 0) {
    try {
      const { fetchNaturalEvents } = await import('../eonet');
      const naturalEvents = await fetchNaturalEvents(_days);
      const wildfires = naturalEvents.filter(e => e.category === 'wildfires');

      detections = wildfires.map(e => ({
        id: e.id,
        location: { latitude: e.lat, longitude: e.lon },
        brightness: 350, // Default for EONET
        frp: 50, // Default for EONET
        confidence: 'FIRE_CONFIDENCE_NOMINAL',
        region: e.title.split(' - ')[0] || 'Global',
        detectedAt: new Date(e.date).getTime(),
        dayNight: 'D',
        satellite: 'EONET',
      }));
    } catch (error) {
      console.error('[Wildfires] EONET fallback failed:', error);
    }
  }

  if (detections.length === 0) {
    return { regions: {}, totalCount: 0, skipped: true, reason: 'NASA_FIRMS_API_KEY not configured and EONET fallback empty' };
  }
