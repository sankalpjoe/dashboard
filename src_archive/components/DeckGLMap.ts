...
export type DeckMapView = 'global' | 'america' | 'mena' | 'eu' | 'asia' | 'southasia' | 'latam' | 'africa' | 'oceania';
...
const VIEW_PRESETS: Record<DeckMapView, { longitude: number; latitude: number; zoom: number }> = {
  global: { longitude: 78, latitude: 22, zoom: 4 },
  southasia: { longitude: 78, latitude: 22, zoom: 4 },
...
export class DeckGLMap {
  ...
  private getTooltip(info: PickingInfo): { html: string } | null {
    ...
    switch (layerId) {
      ...
      case 'ports-layer': {
        const typeIcon = ''; // Removed emojis as requested
